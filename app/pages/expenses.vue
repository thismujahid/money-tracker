...
<template>
  <div class="space-y-8">
    ...
    >
      <div>
        <h2
          class="text-2xl md:text-3xl font-bold text-slate-100 flex items-center gap-2"
        >
          إدارة <span class="gold-gradient-text">المصاريف</span>
        </h2>
        <p class="text-slate-400 mt-1">
          تتبع مصاريفك وسجل كل حاجة بتصرفها خلال يومك بسهولة.
        </p>
      </div>

      <!-- Quick Total Cash Display -->
      <div
        class="glass-panel py-4 px-6 rounded-2xl flex items-center gap-5 border border-gold-500/20 shadow-gold-glow"
      >
        <div
          class="w-12 h-12 rounded-xl bg-gold-500/10 flex items-center justify-center text-gold-400"
        >
          <i class="pi pi-wallet text-2xl"></i>
        </div>
        <div>
          <span class="text-xs text-slate-400 block"
            >رصيد الكاش المتوفر حالياً:</span
          >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="glass-panel rounded-2xl p-6 h-fit relative">
        <h3
          class="text-lg font-bold text-slate-100 mb-5 flex items-center gap-2 pb-3 border-b border-slate-800"
        >
        ...
        <form @submit.prevent="handleAddTransaction" class="space-y-5">
          ...
          <div>
            ...
            >
            <div
              class="grid grid-cols-2 gap-2 bg-slate-900/50 p-1 rounded-xl border border-slate-800"
            >
              <button
                type="button"
                @click="type = 'expense'"
          ...
          <div>
            <label
              for="amount"
              class="block text-xs font-semibold text-slate-400 mb-2"
              >المبلغ (ج.م)</label
            >
            <div class="relative">
              <input
              ...
              />
              <span
                class="absolute top-1/2 right-4 -translate-y-1/2 text-sm text-slate-500 pointer-events-none"
                >ج.م</span
              >
            </div>
          </div>
          ...
          <div>
            <label
              for="source"
              class="block text-xs font-semibold text-slate-400 mb-2"
              >{{type === 'expense'?"صرفتهم فيه إيه يافنان؟":"مصدر الدخل إيه 🧐"}}</label
            >
            <input
              id="source"
              v-model="source"
              type="text"
              required
              :placeholder="`مثال: ${type === 'expense'?'بنزين، بقالة':'راتب الشهر، نقد حالي'}`"
      ...
      <div
        class="glass-panel rounded-2xl p-6 lg:col-span-2 flex flex-col min-h-[400px]"
      >
        <!-- Table Header & Filter Tabs -->
        <div
          class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-slate-800"
        >
          ...
          <!-- Filter Tabs -->
          <div
            class="flex bg-slate-900/50 p-0.5 rounded-xl border border-slate-800 text-xs font-semibold"
          >
            <button
              ...
              :class="
                filterType === 'all'
                  ? 'bg-slate-800 text-gold-400 border border-slate-700/50'
        ...
        <div
          ...
          class="flex-1 flex flex-col items-center justify-center text-center text-slate-500 gap-3 py-12 border border-dashed border-slate-800 rounded-xl"
        >
          <i class="pi pi-search text-3xl text-slate-600"></i>
          <span class="text-base font-semibold"
            >لا توجد سجلات مالية حتى الأن</span
          >
          <p class="text-xs text-slate-400 max-w-xs">
            ضيف بعض المعاملات من نموذج الإضافة لتبدأ في تتبع تفاصيل مصاريفك.
          </p>
        </div>
          ...
          <table class="w-full text-right border-collapse">
            ...
            <tbody class="divide-y divide-slate-850">
              ...
              >
                <!-- Source Name -->
                <td
                  class="py-4 pr-2 font-bold text-slate-200 text-sm max-w-[150px] truncate"
                  :title="t.source"
                >
                  {{ t.source }}
                </td>
